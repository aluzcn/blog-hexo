...
> 该主题已经进�?maintainance mode，这意味着除了修复 bug 之外，不会对该主题进行大范围的修改和新功能的添加�?

## 关于主题

基于ben02移植的[Anatole主题](https://github.com/Ben02/hexo-theme-Anatole)进行了前端重构�?

现在，Anatolo已经是一个类型安全的小巧现代�?Hexo 主题。Anatolo 前端�?js 文件仅仅只有不到 40KB 大小，保证极快的加载速度�?

Anatolo 还在[Anatole主题](https://github.com/Ben02/hexo-theme-Anatole)基础上增加了动态请求、帖子目录、黑暗模式、搜索框等多种理应有的功能，并对界面进行了进一步美化�?

### 鸣谢

搜索框代码大量参考了 [Icarus](https://github.com/ppoffice/hexo-theme-icarus) 的工�?

码云功能和一些中国特色功能来�?[Anatole-core](https://github.com/mrcore/hexo-theme-Anatole-Core)

## 改进

- 引入了现代化前端打包�?Rolldown.js, Typescript �?TSX
- 暗黑模式支持
- 增加了文章概要的选项
- 增加了多语言支持
...
修改hexo根目录下�?`_config.yml` �?`theme: Anatolo`
...
- `.github`: GitHub �?CI 配置文件，用于自动部署样�?
- `includes`: 主题内置�?Hexo 脚本
- `languages`: I18n 文件
- `layout` 主题使用的模板，在服务端（也就是 `hexo g`）渲染成最后的 HTML
- `scripts`: 主题内置�?Hexo 脚本
- `source`: 主题需要的HTML资产
- `src`: 主题前端相关�?typescript 脚本。这些脚本会�?`rolldown` 打包并压缩成一�?`js/complied/bundle.js`

### 代码格式�?

对本主题做出修改后，使用下面的命令可以对代码进行格式�?
